 ...
 * バトルもスマッシュ判定も進行中でない自分のメインフェイズ。ADR-0012）。
 *
 * `'優先権を持っている時'` はキーワード能力「勇気」が既定を上書きしたもので、優先権を持って
 * いること以外は求めない（同 第5部 第2章 2）。バトルやスマッシュ判定の進行中に起動できない
 * のは、起動する権利がそもそも発生していない（同 2 ただし書き、`courage.ts` の
 * `courageRightsOf`）ためであって、行動そのものが禁じられているからではない。トラップの
 * 発動（`play.ts` の `activateTrap`）と同じ形である。
 *
