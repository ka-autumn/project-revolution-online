...
export type ToClient =
  ...
  | {
       ...
       *
       * 前に選んだものが残っていればそれ、無ければ自分のデッキの先頭である。**自分のデッキを
       * 持てない立て方では既製デッキの先頭**で、その時は選ぶところにも既製デッキが並ぶ。
       *
       * **決まらないこともある**（`undefined`）。前に選んだデッキが消えていた時がそれで、選び直す
       * まで席に着けない。**消えたデッキを指さない。**
       *
       * 選べる自分のデッキそのものは `自分のデッキ` で届く。**持ち主にしか送らないものなので、
   ...
   *
   * **繋いだ時にだけ届く**（ADR-0026）。保存・コピー・削除で中身が変わった時は、変わった 1 件
   * だけを `デッキを保存した`・`デッキを消した` で届ける——全件を送り直さない。受け取った側は
   * 覚えておくものに差分を当てればよく、尋ね直す手立ては要らない。ログインを持たない立て方では
   * 届かない。
   */
   ...
   * ——保存は断らないが、確かめはする（同）。
   *
   * **中身をまるごと添える**（ADR-0026）。`自分のデッキ` を送り直さずに一覧へ反映できるように
   * するためで、保存した本人がその場で見るのも、あとから合わせるのも、これ 1 通で足りる。
   */
  | { readonly kind: 'デッキを保存した'; readonly deck: WireOwnedDeck; readonly violations: readonly DeckViolation[] }
  /**
   * 自分のデッキを消した（ADR-0021、ADR-0026）。**持ち主にだけ届く。**
   *
   * `自分のデッキ` を送り直さずに、消えた 1 件だけを伝える。添えるのは識別子だけでよい——
   * 消えたものの中身を、消した後の画面が読み返すことは無い。
   */
  | { readonly kind: 'デッキを消した'; readonly deck: DeckId }
  /**
