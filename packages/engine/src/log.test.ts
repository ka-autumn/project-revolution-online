...
describe('効果の記録', () => {
  ...
  it('選んだものと、その結果が並んで残る', () => {
    const resolved = resolveEffect(board, chooseAndDestroy.effect, { controller: '先攻', chooser: chooseFirst })
  ...
  it('実行されなかった命令は残らない', () => {
    ...
    const resolved = resolveEffect(frozen, freezeEnemies.effect, { controller: '先攻', chooser: chooseFirst })
  ...
  it('能力の解決が、その命令より先に残る', () => {
    ...
    expect(resolved.log.filter((event) => event.kind !== '行動した')).toEqual([
      { kind: '能力を解決した', controller: '先攻', source: drawer.id },
