...
import { cardsIn, cardsOn, findInZone, hasEnded } from './duel.js'
 ...
 * ファザにも、視点ごとに射影した盤面（ADR-0004）を通してこの並びを見せる使い道がある。
 *
 * `優先権を放棄する` は必ず候補に入る（`legalMoves`）。他に行える行動が無くても優先権の放棄
 * だけは行えるため、デュエルが終了していない限り `legalMoves` の結果は空にならない。
 */
export type Move =
...
/** 選択を求められたら常に最初の候補を選ぶ。`legalMoves` が合法性だけを確かめるのに使う。 */
 ...
 * で試す。実際にどれを選ぶかは、これを呼んだ側が `applyMove` に渡す `chooser` で決める。
 *
 * デュエルが終了していれば、それ以上優先権も発生しない（総合ルール 第3部 第3章 3）ので
 * 何も返さない。
 */
export function legalMoves(state: DuelState): readonly Move[] {
  ...
  return [
    ...
    ...cardsIn(state, active, '手札').flatMap((card) =>
      tryMove({ kind: 'エネルギーを置く', card: card.id }, () => placeEnergy(state, card.id)),
    ),
    ...tryMove({ kind: 'プランする' }, () => plan(state, chooseFirst)),
    ...unitsOnSquares(state, active).flatMap((unit) =>
      tryMove({ kind: 'スマッシュする', unit: unit.id }, () => smash(state, unit.id)),
    ),
    ...cardsIn(state, active, 'トラップゾーン').flatMap((card) =>
      tryMove({ kind: 'トラップを廃棄する', card: card.id }, () => discardTrap(state, card.id)),
    ),
    ...playableCards(state, active).flatMap((card) => playCandidates(state, card.id)),
    ...cardsIn(state, active, '手札').flatMap((card) =>
      tryMove({ kind: 'トラップとしてプレイする', card: card.id }, () => playAsTrap(state, card.id)),
    ),
    ...cardsIn(state, priority, 'トラップゾーン').flatMap((card) =>
      tryMove({ kind: 'トラップを発動する', card: card.id }, () => activateTrap(state, card.id, chooseFirst)),
    ),
 ...
 * 実際に選ばれた行動を盤面に適用する。
 *
 * `move` は `legalMoves` が返したものだけを渡す前提であり、その時と盤面が変わっていなければ
 * 必ず成功する。コストの支払いや効果の中の選択は `chooser` に委ねる。
 */
export function applyMove(state: DuelState, move: Move, chooser: Chooser): DuelState {
  switch (move.kind) {
    case '優先権を放棄する':
      return passPriority(state, chooser)
    case 'エネルギーを置く':
      return outcomeState(placeEnergy(state, move.card))
    case 'プランする':
      return outcomeState(plan(state, chooser))
    case 'スマッシュする':
      return outcomeState(smash(state, move.unit))
    case 'トラップを廃棄する':
      return outcomeState(discardTrap(state, move.card))
    case 'カードをプレイする':
      return outcomeState(playCard(state, move.declaration, chooser))
    case 'トラップとしてプレイする':
      return outcomeState(playAsTrap(state, move.card))
    case 'トラップを発動する':
      return outcomeState(activateTrap(state, move.card, chooser))
    case 'ユニットを移動する':
      return outcomeState(moveUnit(state, move.unit, move.destination))
  }
}

/** その行動が試しに行えるなら、その宣言を含む 1 要素の並び。行えなければ空の並び。 */
function tryMove(move: Move, attempt: () => ActionOutcome): readonly Move[] {
  return attempt().kind === '行った' ? [move] : []
}

/** 行えたはずの行動の結果の盤面。`legalMoves` が合法だと確かめた行動でなければ例外になる。 */
 ...
 * 9 つのスクエアそれぞれについて試す。ユニット以外はスクエアを指定しない。
 */
function playCandidates(state: DuelState, card: CardId): readonly Move[] {
  const active = state.turn.active
  const instance = findInZone(state, active, '手札', card) ?? findInZone(state, active, 'プランゾーン', card)
  if (instance === undefined) return []

  if (instance.card.type !== 'ユニット') {
    return tryMove({ kind: 'カードをプレイする', declaration: { card } }, () => playCard(state, { card }, chooseFirst))
  }
  return BATTLE_SPACE.flatMap((square) =>
    tryMove({ kind: 'カードをプレイする', declaration: { card, square } }, () =>
...
function moveCandidates(state: DuelState, unit: CardId): readonly Move[] {
  return BATTLE_SPACE.flatMap((destination) =>
    tryMove({ kind: 'ユニットを移動する', unit, destination }, () => moveUnit(state, unit, destination)),
  )
}
