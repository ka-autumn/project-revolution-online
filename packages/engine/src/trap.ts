 ...
 * 発生を遅らせる規定（同 3-8 ただし書き、第3部 第11章 5、第17章 4）は、まだ実装していない。
 * これは先送りであって、起こらないから省いているのではない。相手のユニットがトリガー
 * アイコンのスクエアに置かれた時にバトルが発生する（ユニットが重なる）ことは実際にあり、
 * いまはその間も権利が残ってしまう。バトル（`battle.ts`）とスマッシュ判定（`smash.ts`）が
 * どちらも盤面に乗った今なら足せるが、権利を得る側の変更なので別の作業として扱う。
 *
