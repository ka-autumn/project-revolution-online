...
import { recipeKeyFromPath, shareKeyFromPath, takePendingRecipe } from './recipe.js'
 ...
 *
 * **WebSocket を張らない。名乗りも部屋も要らない。** ログインしていない人にも開ける——対戦
 * サーバの公開 HTTP の口（`serve.ts`）へ 1 回 `fetch` するだけで完結する（`public-share.ts`）。
 * 指していれば、ふだんの `mount` の手前でここに分岐して終える。
 */
const shareKey = shareKeyFromPath(location.pathname)
if (shareKey !== undefined) {
  void mountPublicShare(root, { serverOrigin: httpOrigin(server), key: shareKey, signInUrl: signInUrl(server) })
} else {
   ...
   *
   * **`takePendingRecipe` は先に呼び、必ず取り出して忘れる。** `??` の右側に直に書いて短絡
   * させると、URL に鍵がある間は預けたものを消さないままになり、次に無関係な理由でログインへ
   * 送られた時に古い鍵を誤って開いてしまう。
   */
