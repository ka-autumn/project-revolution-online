# project-revolution-online

トレーディングカードゲーム「プロジェクトレヴォリューション」を、ルールを完全に自動適用する形で
オンライン対戦できるようにするプロジェクト。

ルールの拠り所は総合ルール Version 15.00 で、全文が [`docs/rules/`](docs/rules/) にある。総合ルールが
「詳細はフロアルールをご参照ください」と外へ投げたところは、フロアルール（Version 1.12）の第2部で、
こちらは [`docs/floor-rules/`](docs/floor-rules/) にある。
用語の正は [`CONTEXT.md`](CONTEXT.md)、設計上の決定は [`docs/adr/`](docs/adr/)。

## 構成

pnpm ワークスペースのモノレポ。

| パッケージ | 役割 |
| --- | --- |
| `packages/engine` | ルールエンジン。「盤面 ＋ 行動 → 次の盤面」の純粋関数。実行時依存を持たず、ブラウザとサーバの両方で動く（ADR-0001） |
| `packages/server` | 盤面の唯一の権威（ADR-0004） |
| `packages/client` | 対戦画面。受け取った盤面を描いて選んだものを送るだけで、ルールの判断は持たない（ADR-0010） |
| `private/cards` | カード実装。この公開リポジトリには含まれない（ADR-0002） |
| `private/decks` | 対戦に使うカードのまとまりを渡すだけ。カードを名指しするため、cards と同じく含まれない |

`private/` は**公開リポジトリに含められないパッケージの置き場**で、独立した非公開リポジトリを
そこへ clone する（ADR-0002）。作業ツリー上はワークスペースの一部として振る舞うが、コミット先が
別になる。

依存の向きは cards → engine の一方向で、engine は cards を知らない。**カードを名指しするのは
`private/decks` だけ**で、デッキの組み方もサーバの立て方も公開側にある（ADR-0002）。

## 開発

Node.js 22 以上。pnpm は `packageManager` で固定しているため、corepack が有効なら追加の準備は要らない。

```sh
pnpm install --frozen-lockfile
pnpm verify
```

`--frozen-lockfile` を付けるのは、`pnpm-lock.yaml` が `private/` 以下の importer を持っているため。
それらが無い環境で素の `pnpm install` を走らせると、ロックファイルから黙って消える。

## 対戦する

サーバと対戦画面を別々に立てる。

```sh
pnpm serve --decks private/decks/src/index.ts   # 対戦サーバ（既定で 8787 番）
pnpm --filter @revolution/client dev            # 対戦画面（既定で 5173 番）
```

**打った対戦は残る**（ADR-0018）。既定では立てたところに `revolution.sqlite` ができ、**サーバを
落として立て直しても、打っていた対戦はそこから戻る。** 残すのは盤面ではなく、デッキとシードと
「行動と答えの並び」だけである。置き先は `--store` で変えられ、空文字を渡せば何も残さずに立つ。

```sh
pnpm serve --decks private/decks/src/index.ts --store ''   # 何も残さない
```

`--decks` に渡すのは、次の 3 つを export するモジュールである（ADR-0021）。
**このリポジトリはどのカードを使うかを知らない**（ADR-0002）ので、実行時に受け取る。

| export | 何を渡すか |
| --- | --- |
| `pool` | 識別子で引けるカードのまとまり。ここに入っているものが、席に持ち込める全部である |
| `presets` | 誰でも使える既製デッキ。`{ id, name, cards }` で、`cards` は識別子の並び |
| `restrictions` | 名前の付いた禁止／制限リスト。`{ id, name, limits }` で、`limits` はカード名ごとの上限（同名のカードを合わせて数える）。空でよい。先頭のものが部屋の既定になる |

識別子は公開側にとってただの文字列で、何を指すかも知らない。デッキは識別子の並びなので、
何をどの枚数入れるかは渡す側が決める。積み方の取り決めはこのリポジトリに無い。立てる時に既製
デッキが構築戦の規定（総合ルール 第3部 第1章 3-1）を満たしているかを確かめるので、満たして
いなければその場で分かる。

**席に着く時に選ぶのは自分のデッキである**（ADR-0021）。既製デッキはコピーして自分のデッキに
してから使うもので、選択肢には並ばない。**ただしデッキを持てるのはログインした人だけ**なので、
上のようにログイン無しで立てた場合は、渡された既製デッキがそのまま選択肢になる。手元で 2 人ぶん
試すときは、席ごとに別の既製デッキを選べる。

あとは `http://localhost:5173` を開くだけでよい。**開くとロビーが出る**（ADR-0017）ので、
そこで部屋を作るか、開いている部屋に入る。

| ロビーで選ぶもの | 何が起きるか |
| --- | --- |
| 人と対戦する | 部屋を作って相手を待つ。作った部屋は、ほかの人のロビーにも出る |
| CPU と対戦する | もう一方の席にサーバが座り、そのまま始まる（ADR-0005 のランダムな手。強くない） |
| 入る | 相手を待っている部屋に入る。2 人揃ったところで始まる |

名乗りは画面が用意してブラウザに覚えさせる（ADR-0017）ので、**閉じても開き直せば同じ席に戻れる。**
そのぶん同じブラウザで開いた 2 つのタブは同じ人になるので、**手元で 2 人ぶん試すときは名乗りを
`?participant=` で分けるか、別のブラウザで開く。**

合言葉を決めて待ち合わせることもできる。URL に付けると、ロビーを通らずその部屋に入る。

```
http://localhost:5173/?participant=あ&room=あいことば
http://localhost:5173/?participant=い&room=あいことば
```

名乗り（`participant`）は認証ではない（ADR-0009）。これを知っている人がその席に座れるので、
切れても同じ名乗りで開き直せば続きから打てる。

**これは手元で立てたときの形である。** ログインを持たせると名乗りは受け付けなくなり、席は
Cookie のセッションで決まる（ADR-0019、「ログインできるようにする」）。

立てるにはカードの実装が要る。無い場合は下を参照。

## 置き場へ出す

**画面は配り（Vercel）、対戦サーバは常駐する 1 台に置く**（ADR-0013、ADR-0014、ADR-0015）。
どちらも同じ出所である必要は無く、繋ぎ先は画面が知っている。

### 対戦サーバを反映する

**カードは置き場でビルドさせない**（ADR-0002）。束ねるのは手元だけで行い、置き場へ渡すのは
出来上がった 1 ファイルにする。

```sh
pnpm build:server --decks private/decks/src/index.ts    # dist/serve.cjs を書き出すだけ
```

運ぶところまでまとめて行うならこちら。**束ねる → 運ぶ → 差し替えて立て直す**を順に行い、
途中で失敗すればそこで止まる。

```sh
pnpm deploy:server --decks private/decks/src/index.ts --host <ユーザ>@<ホスト> --key <秘密鍵>
```

宛先は引数か環境変数で渡す。**このリポジトリは置き場を知らない。**

| 環境変数 | 意味 |
| --- | --- |
| `REVOLUTION_DEPLOY_HOST` | 運ぶ先（`<ユーザ>@<ホスト>`） |
| `REVOLUTION_DEPLOY_KEY` | 使う秘密鍵 |
| `REVOLUTION_DEPLOY_PATH` | 置き場でのパス |
| `REVOLUTION_DEPLOY_UNIT` | 置き場での常駐単位の名前 |

置き場に何を用意するかは ADR-0015 にある。**待つポートは `PORT` で、打った対戦を書く先は `STORE`
で決められる**ので、どちらも束ね直さずに変えられる。

### もう1組（本番以外での通し確認）へ運ぶ

ログイン込みの画面変更を、本番より前に1つ通しで確かめるための、常設のもう1組がある
（ADR-0025）。運び方は同じだが、コマンドと環境変数を分けてある。

```sh
pnpm deploy:server:verify --decks private/decks/src/index.ts --host <ユーザ>@<ホスト> --key <秘密鍵>
```

| 環境変数 | 意味 |
| --- | --- |
| `REVOLUTION_DEPLOY_HOST_VERIFY` | 運ぶ先（`<ユーザ>@<ホスト>`） |
| `REVOLUTION_DEPLOY_KEY_VERIFY` | 使う秘密鍵 |
| `REVOLUTION_DEPLOY_PATH_VERIFY` | 置き場でのパス |
| `REVOLUTION_DEPLOY_UNIT_VERIFY` | 置き場での常駐単位の名前 |

**本番向けの環境変数（`REVOLUTION_DEPLOY_HOST` 等、末尾に `_VERIFY` が付かないもの）とは
名前が別である。** どちらかだけをシェルに設定していても、もう片方のコマンドには拾われない。
引数を省いたときに誤った宛先へ届くことはなく、`pnpm deploy:server` と
`pnpm deploy:server:verify` は常に別々の宛先を向く。

**書く先は、絶対パスで `STORE` に書くこと**（ADR-0018）。既定の `revolution.sqlite` は相対パスで、
**常駐単位の作業ディレクトリから解決される。** それを書いていなければ `/` なので、意図しない
ところにできる。`/run` や `PrivateTmp` の下は作りとして消えるので、そこも選ばない。

置き場所は常駐単位に用意させるのが素直である。`StateDirectory` は**その単位の利用者の持ち物と
して**作られ（ADR-0015 の「専用の利用者で走らせる」とそのまま噛み合う）、立て直しても残る。

```ini
[Service]
StateDirectory=revolution
Environment=STORE=/var/lib/revolution/revolution.sqlite
```

**控えを別のところへ取り、取れていることを確かめる手立ても置く。** 送り続けているものが無いものは、
次に使おうとした時にしか壊れていることが分からない（ADR-0015）。

なお `deploy:server` が差し替えるのは束ねた 1 ファイルだけで、その隣に置いたものを消しはしない。
**それでも同じところに置かない。** 運び方が変われば巻き込まれる。

**初めての宛先には、先に鍵を登録しておく。** `deploy:server` は途中で止まらないように
`BatchMode` で繋ぐので、`known_hosts` に照らせない宛先は「Host key verification failed」で落ちる。

**照らせないのは、宛先ごと無いときだけではない。** ホストは鍵を種類ごとに持っていて、
`known_hosts` に載るのは**その時に選ばれた 1 種類だけ**である。別のクライアント（Windows 標準の
OpenSSH と Git 付属のものなど）が違う種類を選ぶと、宛先を知っていても照らせない。名前と IP も
別の宛先として扱われる。**種類ごと登録しておくのが確実である。**

```sh
ssh-keyscan <ホスト> | ssh-keygen -lf -    # 指紋を確かめる
ssh-keyscan <ホスト> >> ~/.ssh/known_hosts # 確かめてから入れる
```

### 対戦画面を配る

`vercel.json` にビルドの仕方が書いてある。設定するのは 2 つだけで、どちらも無くてよい。

| 環境変数 | 意味 |
| --- | --- |
| `VITE_SERVER_URL` | 繋ぎに行くサーバ（`wss://<サーバ>`）。**ビルド時に焼き付く。** 渡さなければ同じホストの 8787 番 |
| `BASIC_AUTH` | 画面に掛ける鍵（`名前:合言葉`）。渡さなければ素通し |

**`VITE_SERVER_URL` を変えたら、配り直すまで反映されない。** 配った URL を開けばロビーが出る
ので、渡すのはその URL だけでよい。焼いた先とは違うところへ繋ぎたいときは `?server=` で
上書きできる。

```
https://<画面>/
https://<画面>/?participant=あ&room=あいことば   # 合言葉で待ち合わせるとき
https://<画面>/recipe/<鍵>                        # 共有されたレシピを開くとき（ADR-0022）
```

**`/recipe/<鍵>` は画面の側のパスで、開けるのはログインしている人だけである**（ADR-0022）。
`vercel.json` の `rewrite` は、このパスも `index.html` へ回すための手当てで、Vercel に置いた時
だけ効く。**それ以外の静的ホスト（S3、GitHub Pages など、URL のパスをそのままファイルとして
探すもの）に置く場合は、同じ手当て（存在しないパスを `index.html` に回す SPA フォールバック）を
そのホスト側の設定で足すこと。** 足さなければ、共有のリンクを踏んだ人には 404 が返る。手元で
`vite dev` を使う分には、既定の SPA フォールバックがすでに効いている。

**画面に掛けた鍵はサーバには掛かっていない。** ログインを持たせていなければ、**固定した名前を
知っている人は繋げる**（ADR-0014）。そこを閉じるのが次節である。

### ログインできるようにする

**誰であるかを Google から受け取り、席は Cookie のセッションで決まる**（ADR-0019）。下の 4 つを
サーバに渡すと、ログインした人だけが繋げるようになる。**渡さなければ今までどおり `?participant=`
で名乗って入る。両方であることはできず、半分だけ渡すと立ち上がらない。**

| 環境変数 | 意味 |
| --- | --- |
| `GOOGLE_CLIENT_ID` | Google Cloud Console で作った OAuth クライアントの ID |
| `GOOGLE_CLIENT_SECRET` | 同 client secret。**リポジトリに書かず、束ねたものにも焼き込まない** |
| `SIGN_IN_CALLBACK` | Google に登録する折り返し先。`https://<サーバ>/auth/google/callback` |
| `SIGN_IN_RETURN_TO` | 折り返した後に人を送り返す画面の URL |

`SIGN_IN_CALLBACK` に書いた文字列を、そのまま Cloud Console の「承認済みのリダイレクト URI」へ
登録する。**求める scope は `openid` だけ**なので、同意画面でメールアドレスも名前も求めない。
預かるのは Google 上の識別子とセッションと、**本人が付けた表示名**だけである。

**表示名は繋いだ後に本人が決める**（ADR-0020）。初めて繋いだ人はサーバに尋ねられ、決まるまで
部屋を作ることも入ることもできない。決まった名前はロビーと対戦相手のところに出て、いつでも
変えられる。**同じ名前の人が並んでもよい**——席を決めているのは名前ではなく身元である。

**画面とサーバを同じ登録可能ドメインの下に置く**（ADR-0019）。`app.example.com` と
`duel.example.com` のように登録可能ドメインが同じなら、出所が違ってもブラウザは Cookie を握手に
載せる。**別のドメインだと載らないので、ログインは通っても繋げない。**

**ログインを持たせるなら置き場も要る**（`STORE` を空にしない）。身元もセッションもそこに入る。

**サーバは HTTP も同じポートで喋るようになる**（ADR-0019）。中継の設定に、WebSocket だけでなく
通常の HTTP の道も要る。

## 開発の確認

`pnpm verify` は次の 3 つを順に実行する。

```sh
pnpm typecheck       # 全パッケージの型検査
pnpm test            # vitest
pnpm verify:engine   # エンジンが依存ゼロで、ブラウザ／サーバ両方向けにビルドできることの確認
```

## この公開リポジトリだけを clone した場合

`private/` が無いため、対戦を動かすことはできない。`pnpm verify` は通る。
エンジンのテストは実カードではなく架空のテストカードで書くため、カード実装の有無に依存しない（ADR-0002）。
クライアントも同じで、テストは手で組み立てた盤面に対して書くため、カード実装を要らない（ADR-0010）。

この状態では `pnpm install --frozen-lockfile` を使うこと。上に書いたとおり、素の `pnpm install` は
`pnpm-lock.yaml` から非公開パッケージの importer を消し、身に覚えのない差分を残す。
